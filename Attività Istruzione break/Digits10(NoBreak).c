//SENZA USO DELLA FUNZIONE BREAK

/*
*Questo esempio è riportato in pseudocodice nella rivista "2010 - Teaching Abstraction in introductory courses",
*presente all'interno della cartella "Bibliografia per tesi" e mi è sembrato un buon esempio 
*per usare l'espressione and (&&) all'interno delle guardie del ciclo for
*e del ciclo while, senza ricorrere all'uso della funzione break.
 ...
 int main()
{
    ...
    printf("Inserisci:",limit);
     ...
     /*
     * "Trovato" funge da variabile booleana. E' inserita nel ciclo più esterno in quanto se questa vale 1 
     * l'esecuzione termina immediatamente.
     */

     for(i=0;i<limit && trovato == 0;i++)
     {
     //Ad ogni iterazione la variabile somma viene posta a 0.
        somma = 0;

        while(array[i] != 0 && somma != 10)

        {
          ...
          //Verifico se la somma delle cifre è uguale a 10. Se si, pongo trovato a 1.
           if(somma == 10)
          { 
             trovato = 1;
          }
